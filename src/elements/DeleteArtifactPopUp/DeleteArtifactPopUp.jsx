...
const DeleteArtifactPopUp = ({
  ...
  const handleDelete = useCallback(() => {
    ...
    formRef.getState().values.secrets.forEach(secret => {
    ...
    handleDeleteArtifact(
      ...
      category,
      false,
      formRef.getState().values.deletion_strategy,
      secrets
    ).then(() => {
      setIsConfirmDialogOpen(false)
    })
  ...
  const toggleExtendedDeletionStrategy = value => {
    formRef.change('deletion_strategy', value ? '' : 'metadata-only')

    if (formRef.getState().values.secrets.length > 0) {
      formRef.change('secrets', [])
    }
